...
GMs can paint elevations directly onto a scene grid, and a customzable overlay visible to all players will indicate which squares are elevated. Tokens that move onto these squares will automatically have their elevations adjusted to match the terrain, which enables "High Ground" edges on attacks against targets below you.
